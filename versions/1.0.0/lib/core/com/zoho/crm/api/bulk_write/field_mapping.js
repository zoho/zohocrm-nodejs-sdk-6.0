...
class FieldMapping{
	...
	async setDefaultValue(defaultValue)	{
		const DefaultValue = (await (import("./default_value.js"))).MasterModel;
		if((defaultValue != null) && (!(defaultValue instanceof DefaultValue)))	{
	 ...
	 * @returns {number} A number representing the parentColumnIndex
	 */
	 ...
	 * @param {number} parentColumnIndex A number representing the parentColumnIndex
	 */
	setParentColumnIndex(parentColumnIndex)	{
		if((parentColumnIndex != null) && (!(Object.prototype.toString.call(parentColumnIndex) == "[object Number]")))	{
			throw new SDKException(Constants.DATA_TYPE_ERROR, "KEY: parentColumnIndex EXPECTED TYPE: number", null, null);
		}
