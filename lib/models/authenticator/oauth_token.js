...
class OAuthToken extends Token {
  ...
  async getToken() {
    ...
    oauthToken = null;
    if (this.getId() != null) {
      oauthToken = await store.findTokenById(this.getId());
    ...
    if (oauthToken.getAccessToken() == null || oauthToken.getAccessToken().length == 0) {
      if (oauthToken.getRefreshToken() != null && oauthToken.getRefreshToken().length > 0) {
        ...
        await oauthToken.refreshAccessToken(oauthToken, url);
      }
      else {
      ...
      let refreshtoken = await oauthToken.refreshAccessToken(oauthToken, url);
      ...
      try {
        if(oauthToken.getUserSignature() == null && oauthToken.findUser) {
            try {
              ...
              let userName = await utility.getUserName(oauthToken.accessToken);
              if(userName != null) {
  ...
  async authenticate(urlConnection) {
    if(!urlConnection.getHeaders().hasOwnProperty(Constants.AUTHORIZATION)) {
